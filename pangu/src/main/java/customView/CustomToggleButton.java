...
/**
 * Created by yangzw on 2019/4/13 下午10:59.
 */

public class CustomToggleButton extends AppCompatButton {
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = getTypedArray(context, attributeSet, R.styleable.CustomToggleButton);
        if (attr == null) {
            return;
        }
        ...
        try {
            ...
            mIdleText = attr.getString(R.styleable.CustomToggleButton_cpb_textIdle);
            mCompleteText = attr.getString(R.styleable.CustomToggleButton_cpb_textComplete);
            mErrorText = attr.getString(R.styleable.CustomToggleButton_cpb_textError);
            mProgressText = attr.getString(R.styleable.CustomToggleButton_cpb_textProgress);

            mIconComplete = attr.getResourceId(R.styleable.CustomToggleButton_cpb_iconComplete, 0);
            mIconError = attr.getResourceId(R.styleable.CustomToggleButton_cpb_iconError, 0);
            mCornerRadius = attr.getDimension(R.styleable.CustomToggleButton_cpb_cornerRadius, 0);
            mPaddingProgress = attr.getDimensionPixelSize(R.styleable.CustomToggleButton_cpb_paddingProgress, 0);

            int blue = getColor(R.color.cpb_blue);
            int white = getColor(R.color.cpb_white);
            int grey = getColor(R.color.cpb_grey);

            int idleStateSelector = attr.getResourceId(R.styleable.CustomToggleButton_cpb_selectorIdle,
                    R.color.cpb_idle_state_selector);
            mIdleColorState = getResources().getColorStateList(idleStateSelector);

            int completeStateSelector = attr.getResourceId(R.styleable.CustomToggleButton_cpb_selectorComplete,
                    R.color.cpb_complete_state_selector);
            mCompleteColorState = getResources().getColorStateList(completeStateSelector);

            int errorStateSelector = attr.getResourceId(R.styleable.CustomToggleButton_cpb_selectorError,
                    R.color.cpb_error_state_selector);
            mErrorColorState = getResources().getColorStateList(errorStateSelector);

            mColorProgress = attr.getColor(R.styleable.CustomToggleButton_cpb_colorProgress, white);
            mColorIndicator = attr.getColor(R.styleable.CustomToggleButton_cpb_colorIndicator, blue);
            mColorIndicatorBackground =
                    attr.getColor(R.styleable.CustomToggleButton_cpb_colorIndicatorBackground, grey);
        } finally {
            attr.recycle();
        }
    }
    ...
    private OnAnimationEndListener mProgressStateListener = new OnAnimationEndListener() {
        ...
        public void onAnimationEnd() {
            ...
            mStateManager.checkState(CustomToggleButton.this);
        }
    };
    ...
    /**
     * 设置视图的背景。掩盖在Jelly Bean中进行的API更改.
     */
